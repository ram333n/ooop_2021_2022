...
public:
	Matrix() = default;
	Matrix(size_t rows_, size_t columns_ = rows_);
	...
	[[nodiscard]] friend Matrix operator+(const Matrix<T>& lhs, const Matrix<T>& rhs);
	[[nodiscard]] friend Matrix operator-(const Matrix<T>& lhs, const Matrix<T>& rhs);
...
[[nodiscard]] Matrix<T> operator+(const Matrix<T>& lhs, const Matrix<T>& rhs) {
	...
	Matrix result(lhs.rows_count, lhs.columns_count);
